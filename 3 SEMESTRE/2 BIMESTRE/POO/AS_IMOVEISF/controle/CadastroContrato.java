...
public class CadastroContrato {
    ...
    private static DaoImoveis daoImoveis = new DaoImoveis();
    ...
    public static void setarInformacoes(Contrato contrato) {
        ...
        contrato.setFuncionario(daoVendedor.carregarPorCPF(cpf));
        ...
        cpf = Input.next();
        contrato.setCliente(daoCliente.carregarPorCPF(cpf));
        ...
        contrato.setImovel(daoImoveis.carregarPorMatricula(matricula));
    ...
    public static void remover(Contrato p) {
        if (contratos.remove(p)) {
            System.out.println("O contrato id:" + p.getId() + " cliente: " + p.getCliente().getNome()
